...
class PromptAnalyticsControllerIntegrationTest {
    ...
    void setupTestData() throws Exception {
        ...
        // Publish the prompt
        mockMvc.perform(post("/prompts/" + publicPromptId + "/publish")
                .cookie(new Cookie("token", authToken)))
                .andExpect(status().isOk());
        ...
        result = mockMvc.perform(post("/prompts")
                .cookie(new Cookie("token", authToken))
                .contentType(MediaType.APPLICATION_JSON)
        ...
        Prompt createdFeaturedPrompt = objectMapper.readValue(result.getResponse().getContentAsString(), Prompt.class);
        featuredPromptId = createdFeaturedPrompt.getId();
        
        // Publish the featured prompt
        mockMvc.perform(post("/prompts/" + featuredPromptId + "/publish")
                .cookie(new Cookie("token", authToken)))
                .andExpect(status().isOk());
    }
    ...
    void whenGetTrendingPrompts_thenReturnList() throws Exception {
        mockMvc.perform(get("/api/analytics/trending"))
                .andExpect(status().isOk())
    ...
    void whenGetFeaturedPrompts_thenReturnList() throws Exception {
        mockMvc.perform(get("/api/analytics/featured"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
    ...
    void whenGetTopRankingPrompts_thenReturnList() throws Exception {
        mockMvc.perform(get("/api/analytics/top-ranking"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray());
    }
