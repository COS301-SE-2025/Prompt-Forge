...
class CartItemControllerTest {
    ...
    void addItemToCart_ShouldReturnConflictIfAlreadyAdded() {
        ...
        doThrow(new RuntimeException("already been added")).when(cartItemService).addItemToCart(userId, promptId);
    ...
    void addItemToCart_ShouldReturnServerErrorOnOtherException() {
        CartItemRequest request = mock(CartItemRequest.class);
        when(request.getPromptId()).thenReturn(promptId);
        doThrow(new RuntimeException("other error")).when(cartItemService).addItemToCart(userId, promptId);
        ResponseEntity<CartItemResponse> response = controller.addItemToCart(request, authentication);
    ...
    void removeItemFromCart_ShouldReturnOk() {
        ...
        ResponseEntity<CartItemResponse> response = controller.removeItemFromCart(promptId, authentication);
        assertEquals(200, response.getStatusCodeValue());
    ...
    void removeItemFromCart_ShouldReturnServerErrorOnException() {
        doThrow(new RuntimeException("fail")).when(cartItemService).removeItemFromCart(userId, promptId);
        ResponseEntity<CartItemResponse> response = controller.removeItemFromCart(promptId, authentication);
        assertEquals(500, response.getStatusCodeValue());
    }
