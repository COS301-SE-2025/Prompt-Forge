...
class OpenRouterTestControllerWebTest {
	...
	void chat_streamTrue_redirectsToStreamEndpoint() throws Exception {
		...
		mockMvc
				.perform(post("/api/test/openrouter/chat").contentType(MediaType.APPLICATION_JSON).content(body))
				.andExpect(status().isMovedPermanently())
				.andExpect(header().string("Location", "/api/test/openrouter/chat/stream"))
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
				.andExpect(
						content().json(
								"{\"message\":\"Use /chat/stream endpoint for streaming requests\"}"));
	}
	...
	void chat_regular_serverError_503_userMessage() throws Exception {
		...
		mockMvc
				...
				.andExpect(content().string(Matchers.containsString("The model abc is currently unavailable")));
	}
	...
	void chatStream_missingModel_returnsSseError() throws Exception {
		...
		mockMvc
				.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.asyncDispatch(mvcResult))
	...
	void chatStream_upstreamError_returnsSseError() throws Exception {
		...
		String body = "{\"model\":\"abc\",\"stream\":true,\"messages\":[{\"role\":\"user\",\"content\":\"Hi\"}]}";
		MvcResult mvcResult =
				mockMvc
						.perform(post("/api/test/openrouter/chat/stream").contentType(MediaType.APPLICATION_JSON).content(body))
						.andExpect(request().asyncStarted())
						.andReturn();
	...
	void chatStream_missingChoicesKey_skipped() throws Exception {
		...
		mockMvc
				...
				.andExpect(content().string(Matchers.containsString("connection established")))
				.andExpect(content().string(Matchers.containsString("data: [DONE]")))
				.andExpect(content().string(Matchers.not(Matchers.containsString("data: {\"other\":\"value\"}"))));
	}
	...
	void chatStream_emptyChoicesArray_skipped() throws Exception {
		...
		mockMvc
				...
				.andExpect(content().string(Matchers.containsString("connection established")))
				.andExpect(content().string(Matchers.containsString("data: [DONE]")))
				.andExpect(content().string(Matchers.not(Matchers.containsString("data: {\"choices\":[]}"))));
	}
	...
	void chatStream_missingDeltaKey_skipped() throws Exception {
		...
		mockMvc
				...
				.andExpect(content().string(Matchers.containsString("connection established")))
				.andExpect(content().string(Matchers.containsString("data: [DONE]")))
				.andExpect(content().string(Matchers.not(Matchers.containsString("data: {\"choices\":[{\"other\":\"value\"}]}"))));
	}

	@Test
	void chatStream_missingContentKey_skipped() throws Exception {
		String sseBody = "data: {\"choices\":[{\"delta\":{\"other\":\"value\"}}]}\n\n" + "data: [DONE]\n\n";
		startMockOpenRouter(200, "text/event-stream", sseBody.getBytes(StandardCharsets.UTF_8));
		...
		mockMvc
				...
				.andExpect(content().string(Matchers.containsString("connection established")))
				.andExpect(content().string(Matchers.containsString("data: [DONE]")))
				.andExpect(content().string(Matchers.not(Matchers.containsString("data: {\"choices\":[{\"delta\":{\"other\":\"value\"}}]}"))));
	}

	@Test
	void chatStream_nullContentValue_skipped() throws Exception {
		String sseBody = "data: {\"choices\":[{\"delta\":{\"content\":null}}]}\n\n" + "data: [DONE]\n\n";
		startMockOpenRouter(200, "text/event-stream", sseBody.getBytes(StandardCharsets.UTF_8));
		...
		mockMvc
				...
				.andExpect(content().string(Matchers.containsString("connection established")))
				.andExpect(content().string(Matchers.containsString("data: [DONE]")))
				.andExpect(content().string(Matchers.not(Matchers.containsString("data: {\"choices\":[{\"delta\":{\"content\":null}}]}"))));
	}

	@Test
	void chatStream_emptyContentString_skipped() throws Exception {
		String sseBody = "data: {\"choices\":[{\"delta\":{\"content\":\"\"}}]}\n\n" + "data: [DONE]\n\n";
		startMockOpenRouter(200, "text/event-stream", sseBody.getBytes(StandardCharsets.UTF_8));
		...
		mockMvc
				...
				.andExpect(content().string(Matchers.containsString("connection established")))
				.andExpect(content().string(Matchers.containsString("data: [DONE]")))
				.andExpect(content().string(Matchers.not(Matchers.containsString("data: {\"choices\":[{\"delta\":{\"content\":\"\"}}]}"))));
	}

	@Test
	void chatStream_validContentWithText_forwarded() throws Exception {
		String sseBody = "data: {\"choices\":[{\"delta\":{\"content\":\"Hello World\"}}]}\n\n" + "data: [DONE]\n\n";
		startMockOpenRouter(200, "text/event-stream", sseBody.getBytes(StandardCharsets.UTF_8));
		...
		mockMvc
				.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.asyncDispatch(mvcResult))
				.andExpect(status().isOk())
				.andExpect(content().string(Matchers.containsString("data: {\"choices\":[{\"delta\":{\"content\":\"Hello World\"}}]}")))
				.andExpect(content().string(Matchers.containsString("data: [DONE]")));
	}
	...
	void chatStream_multipleValidContentChunks_allForwarded() throws Exception {
		...
		mockMvc
				.perform(org.springframework.test.web.servlet.request.MockMvcRequestBuilders.asyncDispatch(mvcResult))
				.andExpect(status().isOk())
				.andExpect(content().string(Matchers.containsString("data: {\"choices\":[{\"delta\":{\"content\":\"Hello\"}}]}")))
				.andExpect(content().string(Matchers.containsString("data: {\"choices\":[{\"delta\":{\"content\":\" \"}}]}")))
				.andExpect(content().string(Matchers.containsString("data: {\"choices\":[{\"delta\":{\"content\":\"World\"}}]}")))
				.andExpect(content().string(Matchers.containsString("data: [DONE]")));
	}
} 