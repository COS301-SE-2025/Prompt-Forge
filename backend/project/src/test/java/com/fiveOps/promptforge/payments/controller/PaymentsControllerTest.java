...
import com.fiveOps.promptforge.payments.dto.*;
...
public class PaymentsControllerTest {
    ...
    void testinitializePaymentSuccess() throws Exception {
        ...
        TransactionInitializationResponse response = new TransactionInitializationResponse(1000, TEST_EMAIL, "ref_123");

        lenient().when(paymentService.initializePayment(TEST_EMAIL, prompts, 100.0))
                .thenReturn(response);

        // Act
        ResponseEntity<APIResponse> result = paymentsController.initializePayment(request, authentication);
    ...
    void testGetUserPayoutDetailsSuccess() {
        // Arrange
        PayoutCardWithSubaccountCodeDTO detailsWithCode = new PayoutCardWithSubaccountCodeDTO(
    ...
    void testAddPayoutCardSuccess() throws Exception {
        // Arrange
        PayoutCardDTO request = new PayoutCardDTO(
                "123", "Test Bank", "1234567890", "Test User");
        UserDto userDto = new UserDto();
        userDto.setUsername("testuser");

        PaystackAddSubaccountResponseDTO paystackResponse = new PaystackAddSubaccountResponseDTO(true, "SUB_123");
    ...
    void testUpdatePayoutCardSuccess() throws Exception {
        ...
        lenient().when(userService.getUserByEmail(TEST_EMAIL)).thenReturn(userDto);
        lenient().when(bankDetailsService.getSubaccountCodeByUserID(TEST_USER_ID))
                .thenReturn("SUB_123");
