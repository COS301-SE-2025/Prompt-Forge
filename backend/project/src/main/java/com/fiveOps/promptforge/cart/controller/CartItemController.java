...
public class CartItemController {
    ...
    public ResponseEntity<CartItemResponse> addItemToCart(@RequestBody CartItemRequest request, 
        ...
        try {
            ...
            if (e.getMessage().contains("already been added")) {
                return ResponseEntity.status(HttpStatus.CONFLICT).body(new CartItemResponse("Prompt added to cart."));
            }
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new CartItemResponse("Failed to add item to cart."));
        }
    }

    @DeleteMapping("/remove/{promptId}")
    public ResponseEntity<CartItemResponse> removeItemFromCart(@PathVariable UUID promptId,
            Authentication authentication) {
    ...
    public ResponseEntity<Boolean> isPromptAddedToCart(@PathVariable UUID promptId, Authentication authentication) {
        String userEmail = authentication.getName();
        UUID userId = userService.getUserIdByEmail(userEmail);
    ...
    public ResponseEntity<CartItemResponse> checkoutCart(@RequestBody CartCheckoutRequest request,
            Authentication authentication) {
        try {
            String userEmail = authentication.getName();
            UUID userId = userService.getUserIdByEmail(userEmail);
            ...
            e.printStackTrace();
            return ResponseEntity.badRequest().body(new CartItemResponse("Checkout failed: " + e.getMessage()));
        }
    }
}
