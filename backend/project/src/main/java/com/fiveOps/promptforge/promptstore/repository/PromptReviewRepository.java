...
public interface PromptReviewRepository extends JpaRepository<PromptReview, UUID> {
    ...
    @Modifying
    @Query("UPDATE PromptReview r SET r.rating = :rating, r.comment = :comment " +
        "WHERE r.id = :reviewId AND r.userId = :userId AND r.promptId = :promptId")
    int updateReview(@Param("reviewId") UUID reviewId,
    ...
    @Modifying
    @Query("DELETE FROM PromptReview r " +
        "WHERE r.id = :reviewId AND r.userId = :userId AND r.promptId = :promptId")
    int deleteByReviewIdAndUserIdAndPromptId(@Param("reviewId") UUID reviewId,
                                        @Param("userId") UUID userId,
                                        @Param("promptId") UUID promptId);
