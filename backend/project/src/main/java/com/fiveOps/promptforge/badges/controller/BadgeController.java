...
public class BadgeController {
      ...
      for (Cookie cookie : request.getCookies()) {
        if (cookie.getName().equals("token")) {
          ...
          if (token != null && jwtUtil != null) {
            return jwtUtil.extractUsername(token); // email stored as subject
          }
        }
      }
  ...
  public ResponseEntity<Map<String, Object>> checkAndAwardMyBadges(HttpServletRequest request) {
    try {
      ...
      if (badgeAwardingService != null) {
        badgeAwardingService.checkAndAwardAllBadges(user.getUserId());
      } else {
        System.err.println("BadgeAwardingService is not available");
        return ResponseEntity.status(503)
            .body(Map.of("error", "Badge service temporarily unavailable"));
      }

      // Get updated badge count
      Long newCount = badgeService != null ? badgeService.getUserBadgeCount(user.getUserId()) : 0L;
