...
public class CartItemService {
    ...
    public CartItemService(CartItemRepository cartItemRepository, UserRepository userRepository, PromptRepository promptRepository,PromptStoreService promptStoreService) {
    ...
    public Page<CartItemDTO> getCartItemsForUser(UUID userId, Pageable pageable) {
        ...
        List<CartItemDTO> dtos = results.stream().map(row -> {
            ...
            return new CartItemDTO(cartItemId, fetchedUserId, username, promptId, promptTitle, promptTags, promptPrice);
        }).toList();
    ...
    public void checkout(UUID userId, List<CartItemDTO> prompts) {
        ...
        try {
            for (int i = 0; i < prompts.size(); i++) {
                ...
                System.out.println("Found prompt: " + prompt.getTitle() + " (Price: " + prompt.getPrice() + ")");
