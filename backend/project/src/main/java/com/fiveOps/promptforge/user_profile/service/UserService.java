...
public class UserService {
  ...
  public List<UserDto> getFollowersByEmail(String email) {
    ...
    List<UUID> followerIds =
        user.getFollowers() != null ? Arrays.asList(user.getFollowers()) : List.of();
  ...
  public List<UserDto> getAllUsersExceptCurrent(String currentUserEmail) {
    return userRepository.findAllByEmailNot(currentUserEmail).stream()
        .sorted(
            (user1, user2) -> {
  ...
  private int calculateCompletenessScore(User user) {
    ...
    if (user.getUsername() != null && !user.getUsername().isEmpty()) score += 10;
    if (user.getBio() != null && !user.getBio().isEmpty()) score += 10;
    if (user.getProfilePictureUrl() != null && !user.getProfilePictureUrl().isEmpty()) score += 10;

    // Social connections
    if (user.getFollowers() != null && user.getFollowers().length > 0) score += 5;
    if (user.getFollowing() != null && user.getFollowing().length > 0) score += 5;
  ...
  public List<UserDto> fuzzySearchUsers(String query) {
    ...
    return allUsers.stream()
        ...
        .sorted(Comparator.comparingInt(user -> -calculateRelevanceScore(user, searchTerms)))
        .collect(Collectors.toList());
  }
  ...
  private boolean matchesSearchTerms(UserDto user, String[] terms) {
    ...
    return Arrays.stream(terms).allMatch(term -> username.contains(term) || bio.contains(term));
  }
